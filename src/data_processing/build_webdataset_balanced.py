...
def parse_args():
    ...
    ap.add_argument("--limit", type=int, default=0,
                    help="If >0, limit the number of processed samples per split (debug)")
    ap.add_argument("--skip-existing", action="store_true",
                    help="If provided, don't regenerate shards when they already exist (same pattern)")
    ...
    for subset in args.splits:
        ...
        for it in pbar:
            try:
                ...
                if not src:
                raise ValueError("Missing source_abs_path/source_rel_path")
