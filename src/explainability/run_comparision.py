...
Inputs:
  ...
  NO-ROI (model-independent, canonical):
    - NO_ROI_PATHS.root_dir / artifacts/
        scores_fp32.npy
        keys.npy
        selected_concepts.json

  ROI (per model, produced by run_spatial-concept.py):
    ...
    - summary_json contains "items", each with:
        key
        concept_scores_json (relative path to heavy_run_dir)

Canonical outputs:
  - XAI_ROOT/roi-no_roi-comparison/<MODEL_ID>/   (back-compat: also accepts '.../roi-no_roi-comparision/')
...
def _canon_key_strip_prefix(k: str) -> str:
    """
    Canonicalize keys like 'test::abcd' -> 'abcd' (as in run_spatial-concept).
    """
    s = str(k)
    if "::" in s:
...
def _load_latest_roi_summary(model_id: str, log: logging.Logger) -> Tuple[Path, List[Dict[str, Any]]]:
    """
    Find heavy_run_dir + list of ROI items starting from output/roi/<MODEL_ID>/spatial_concept/latest_run.json
    """
    ...
    if not latest_path.exists():
        raise FileNotFoundError(
            f"latest_run.json not found for ROI: {latest_path} "
            "(run run_spatial-concept.py first)"
        )

    latest = json.loads(latest_path.read_text())
    heavy_run_dir = Path(latest.get("heavy_run_dir", "")).expanduser()
    if not heavy_run_dir.exists():
        raise FileNotFoundError(f"heavy_run_dir does not exist: {heavy_run_dir}")
    ...
    if not summary_json_path.exists():
        raise FileNotFoundError(f"summary_json not found: {summary_json_path}")

    summary = json.loads(summary_json_path.read_text())
    items = summary.get("items", [])
    if not isinstance(items, list) or not items:
        raise RuntimeError(f"xai_summary.json empty or invalid: {summary_json_path}")
    log.info("Found %d ROI items (summary=%s)", len(items), summary_json_path)
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        type=Path,
        required=True,
        help="Root of an ablation (e.g: .../exp_*/exp_*_abl01)",
    )
    ap.add_argument(
        "--no-roi-root",
        type=Path,
        default=NO_ROI_PATHS.root_dir,
        help="Canonical NO-ROI root (default: NO_ROI_PATHS.root_dir).",
    )
    ap.add_argument(
        "--config",
        type=Path,
        default=_default_comp_cfg(),
        help="Config YAML (optional, default: explainability/configs/comparison.yaml)",
    )
    ap.add_argument(
        "--model-id",
        type=str,
        default=None,
        help="Override model id (default: folder name of model-root).",
    )
...
def main() -> None:
    ...
    for p in (no_scores_p, no_keys_p, sel_json_p):
        if not p.exists():
            raise FileNotFoundError(f"Missing NO-ROI artifact: {p}")
    ...
    if no_scores.ndim != 2:
        raise RuntimeError(f"NO-ROI scores_fp32 must be 2D, shape={no_scores.shape}")
    ...
    for row in items:
        ...
        if not cs_path.exists():
            log.warning("concept_scores.json missing for key=%s: %s", key, cs_path)
            missing_scores += 1
            continue
        ...
        try:
            ...
            log.warning("concept_scores.json unreadable (%s): %s", cs_path, e)
            missing_scores += 1
            continue
    ...
    if not roi_vecs:
        raise RuntimeError(
            f"No valid ROI vector found (items={len(items)}, missing_scores={missing_scores})."
        )
    ...
    if not aligned_keys:
        raise RuntimeError(
            f"No common keys between ROI and NO-ROI. "
    ...
    log.info("Comparison done: %s", lay.root_dir)
